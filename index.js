...
app.post("/signup", async (req, res) => {
  ...
  const validatePassword = bcrypt.compareSync("nn22", passwordHash);
  console.log(passwordHash, validatePassword);

  try {
    //verificando se já existe email/senha
    ...
    console.log(passwordList, existingPassword);
    if(existingPassword.length !== 0){
        return res.status(409).send('Senha já existente');
    }
    //adicionando ao db
